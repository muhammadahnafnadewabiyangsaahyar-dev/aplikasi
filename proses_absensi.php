...
try {
    ...
    if ($tipe_absen == 'masuk') {
        if (preg_match('/^data:image\/(\w+);base64,/', $foto_base64, $type)) {
            ...
            // Standarisasi nama file dengan tanggal untuk mencegah collision
            $nama_file_foto = 'masuk_' . $user_id . '_' . $tanggal_hari_ini . '_' . time() . '.' . ($type == 'jpeg' ? 'jpg' : $type);
            $folder_masuk = 'uploads/absensi/';
    ...
    if ($tipe_absen == 'masuk') {
        ...
        if ($is_admin) {
            // ========================================================
            // ADMIN: Tidak ada keterlambatan, tidak ada potongan
            // Admin tidak terikat shift, jadi selalu "tepat waktu"
            // ========================================================
            $menit_terlambat = 0;
            ...
            if ($selisih_detik < $toleransi_awal_detik) {
                ...
                if ($menit_terlambat > 0 && $menit_terlambat < 20) {
                    ...
                    // Level 3: Terlambat 40+ menit (tapi masih dalam 12 jam)
        ...
        if (!empty($foto_base64)) {
            if (preg_match('/^data:image\/(\w+);base64,/', $foto_base64, $type)) {
                ...
                // Standarisasi nama file dengan tanggal untuk mencegah collision
                $nama_file_foto_keluar = 'keluar_' . $user_id . '_' . $tanggal_hari_ini . '_' . time() . '.' . ($type == 'jpeg' ? 'jpg' : $type);
                $folder_keluar = 'uploads/absensi/';
        ...
        $sql_shift = "SELECT c.jam_masuk, c.jam_keluar, c.id as cabang_id
                      ...
                      LIMIT 1";
        $stmt_shift = $pdo->prepare($sql_shift);
        $stmt_shift->execute([$user_id, $tanggal_absensi]);
        $shift_info = $stmt_shift->fetch(PDO::FETCH_ASSOC);
        
        // Jika tidak ada shift assignment yang confirmed, coba ambil dari cabang user
        if (!$shift_info) {
            $sql_shift_default = "SELECT c.jam_masuk, c.jam_keluar, r.id_cabang as cabang_id
                                  FROM register r
                                  JOIN cabang c ON r.id_cabang = c.id
                                  WHERE r.id = ?
                                  LIMIT 1";
            $stmt_shift_default = $pdo->prepare($sql_shift_default);
            $stmt_shift_default->execute([$user_id]);
            $shift_info = $stmt_shift_default->fetch(PDO::FETCH_ASSOC);
        }
