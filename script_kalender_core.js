...
(function(window) {
    ...
    function generateDayView(date) {
        ...
        contentHeader.className = 'day-header';
        const monthNames = window.KalenderUtils.monthNames;
        contentHeader.textContent = `${date.getDate()} ${monthNames[date.getMonth()]} ${date.getFullYear()}`;
        contentHeader.style.cssText = 'height: 50px; display: flex; align-items: center; justify-content: center; font-weight: bold; background-color: #f9f9f9; border-bottom: 2px solid #ddd;';
    ...
    function generateYearView(year) {
        ...
        yearHeader.style.cssText = 'grid-column: 1 / -1; text-align: center;';
        yearHeader.textContent = `Tahun ${year}`;
        yearGrid.appendChild(yearHeader);
