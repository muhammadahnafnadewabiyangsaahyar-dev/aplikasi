    ...
    try {
        ...
        if ($register_id) {
            ...
            if ($id_gaji) {
                ...
                $stmt = $pdo->prepare("UPDATE komponen_gaji SET jabatan=?, gaji_pokok=?, tunjangan_transport=?, tunjangan_makan=?, overwork=?, tunjangan_jabatan=?, bonus_kehadiran=?, bonus_marketing=?, insentif_omset=? WHERE id=?");
                $stmt->execute([$posisi, $gaji_pokok, $tunjangan_transport, $tunjangan_makan, $overwork, $tunjangan_jabatan, $bonus_kehadiran, $bonus_marketing, $insentif_omset, $id_gaji]);
            } else {
...
$stmt = $pdo->prepare("
    SELECT 
        ...
        COALESCE(kg.bonus_kehadiran, pw.bonus_kehadiran) as bonus_kehadiran,
        COALESCE(kg.bonus_marketing, pw.bonus_marketing) as bonus_marketing,
        COALESCE(kg.insentif_omset, pw.insentif_omset) as insentif_omset,
        r.id as register_id
    ...
    <div class="form-container">
        ...
        <form method="post" action="edit_pegawai.php?nama=<?= urlencode($nama_lengkap) ?>">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label for="bonus_kehadiran">Bonus Kehadiran (Rp)</label>
                    <input type="number" step="0.01" name="bonus_kehadiran"
                           value="<?= $pegawai['bonus_kehadiran'] ?? '' ?>" placeholder="0">
                </div>
            </div>
