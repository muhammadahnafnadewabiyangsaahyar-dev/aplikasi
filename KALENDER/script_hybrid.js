        ...
        for (let j = 0; j < 7; j++) {
            if (i === 0 && j < firstDay) {
                ...
                cell.addEventListener('click', () => {
                    if (useDatabaseMode && !currentCabangId) {
                        alert('Pilih cabang terlebih dahulu untuk database mode');
                        return;
                    }
                    openModal(dateKey, cell);
                });
...
function generateWeekView(date) {
    ...
    if (!timeColumn || !daysColumn || !weekRange) return;
    ...
    for (let i = 0; i < 7; i++) {
        ...
        dayHeader.textContent = dayDate.toLocaleDateString('id-ID', { weekday: 'short', day: 'numeric' });
...
function generateDayView(date) {
    ...
    const dayDateSpan = document.getElementById('day-date');

    if (!timeColumn || !dayContent || !dayDateSpan) return;

    timeColumn.innerHTML = '';
    dayContent.innerHTML = '';

    dayDateSpan.textContent = date.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
